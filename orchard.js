...
// CODE HERE
let totalAcres = 0; //create a mutable variable called totalAcres
...
//for loop loops thru three arrays and incease totalAcres amount by amount at that index in that array
console.log(totalAcres);//prints out totalAcres
...
// CODE HERE
let numberOfDays = 0;//initiate variable numberOfDays
numberOfDays += fujiAcres.length;
numberOfDays += galaAcres.length;
numberOfDays += pinkAcres.length;//total of numberOfDays by adding length of three arrays up
let averageDailyAcres = 0;
averageDailyAcres = totalAcres/numberOfDays;
...
}//as long as acresLeft is greater than 0, variable days will increase by 1, acresLeft will be updated too
console.log(days);
...
let pinkTons = [];
for (let i = 0; i < fujiAcres.length; i++){
    fujiAmount =fujiAcres[i]*6.5;
    fujiTons.push(fujiAmount);
}
for (let i = 0; i < galaAcres.length; i++){
    galaAmount =galaAcres[i]*6.5;
    galaTons.push(galaAmount);
}
for (let i = 0; i < pinkAcres.length; i++){
    pinkAmount =pinkAcres[i]*6.5;
    pinkTons.push(pinkAmount);
}//loop thru each acres array, times each acre yield, and add the result to each tons array
...
}//loop thru fujitons array to know total amount of fuji
let totalFujiPounds = 0;
totalFujiPounds = 2000*totalFuji;// total fuji times 2000 pounds convertion gives total fuji in pounds
console.log(totalFujiPounds);

let totalGala = 0;
for (let i = 0; i < galaTons.length; i++){
    totalGala +=galaTons[i];
}//loop thru galatons array to know total amount of gala
let totalGalaPounds = 0;
totalGalaPounds = 2000*totalGala;//total gala times 2000 pounds convertion gives total gala in pounds
console.log(totalGalaPounds);

let totalPink = 0;
for (let i = 0; i < pinkTons.length; i++){
    totalPink +=pinkTons[i];
}//loop thru pinktons array to know total amount of pink
let totalPinkPounds = 0;
totalPinkPounds = 2000*totalPink;//total pink times 2000 pounds convertion gives total pink in pounds
...
let galaProfit = 0;
let pinkProfit = 0;
fujiProfit = fujiPrice*totalFujiPounds;
galaProfit = galaPrice*totalGalaPounds;
pinkProfit = pinkPrice*totalPinkPounds;//each profit = its price times its total pounds
...
// CODE HERE
let totalProfit = 0;
totalProfit = fujiProfit+galaProfit+pinkProfit;//adding all profits up to know the total profit
